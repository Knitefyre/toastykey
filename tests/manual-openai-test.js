...
/**
 * Manual OpenAI Proxy Test Script
 *
 * Usage:
 *   node tests/manual-openai-test.js setup    - Add OpenAI API key to vault
 *   node tests/manual-openai-test.js test     - Test the proxy with a simple API call
 */
...
/**
 * Setup command - Add OpenAI API key to vault
 */
async function setup() {
  console.log('\n=== ToastyKey OpenAI Setup ===\n');
  console.log('This will store your OpenAI API key securely in the vault.');
  ...
  try {
    ...
    // Add key to vault
    const result = await vault.addKey('openai', 'default', apiKey.trim());

    if (result.success) {
      console.log('\n✓ OpenAI API key stored successfully!');
      console.log(`  Key ID: ${result.id}`);
      console.log('\nYou can now run: node tests/manual-openai-test.js test\n');
    } else {
...
async function test() {
  ...
  try {
    ...
    const response = await axios.post(
      `${PROXY_URL}/openai/v1/chat/completions`,
      {
        model: 'gpt-4o-mini',
        messages: [
          {
    ...
    console.log('The OpenAI proxy is working correctly.\n');
...
async function main() {
  ...
  if (!command) {
    console.log('\nUsage:');
    console.log('  node tests/manual-openai-test.js setup    - Add OpenAI API key to vault');
    console.log('  node tests/manual-openai-test.js test     - Test the proxy with a simple API call\n');
    process.exit(1);
  }
